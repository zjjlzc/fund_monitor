...
class Spider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        for url in self.urls:
            yield scrapy.Request(url=url, callback=self.parse0)
    ...
    def parse1(self, response):
        ...
        # 年份列表
        url = "http://fund.eastmoney.com/f10/FundArchivesDatas.aspx?type=jjcc&code=%s&topline=200" %(item['fund_code'])
        html = requests_manager.get_html(url)
        with open('test1.html', 'w') as f:
            f.write(html)
        year_list = eval(re.search(r'(?<=arryear:)\[.+?\](?=,)', html).group()) if re.search(r'(?<=arryear:)\[.+?\](?=,)', html) else None
        if year_list is None:
            raise Exception(u'错误的 url %s' %url)

        for year0 in year_list:
            url = "http://fund.eastmoney.com/f10/FundArchivesDatas.aspx?type=jjcc&code=%s&topline=200&year=%s" %(item['fund_code'],str(year0))
            html = requests_manager.get_html(url)
            with open('test2.html','w') as f:
                f.write(html)

            html = re.search(r"<div class='box'>.+</div>", html).group() if re.search(r"<div class='box'>.+</div>", html) else None
            if html is None:
                raise Exception(u'错误的 url %s' % url)
            ...
            #     df0.columns = [re.sub(r'\s+', '', s) for s in df0.columns]
            #
            #     func = lambda s: re.search(ur'占净值|持股数|', s).group() if re.search(ur'占净值|持股数|持仓市值', s) else s
            #     df0.columns = [func(s) for s in df0.columns]
            #持仓市值
            #     df0[u'标题'] = title
            #     df0[u'cut_off_date'] = title.split(u'截止至：')[-1]
