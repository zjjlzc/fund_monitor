...
class net_value_cal_display(object):
    def __init__(self, method='weekly'):
        self.method = method
    ...
    def weekly_report(self):
        ...
        for code in df.index:
            ...
            df.loc[code, u'基金经理'] = df0[u'基金经理'].iloc[0] + employment_period
            ...
            # 近三个月的月收益率
            for date1 in [datetime.datetime(year=date_today.year,month=(date_today - relativedelta(months=i)).month,day=1) for i in [0,1,2]]:
            ...
            date2 = datetime.datetime(year=date_today.year, month=3, day=31)
            ser_3m_first = self.simple_cal(code, date1, date2, u'收益')
            # 近一年 收益率，回撤，收益回撤比
            ...
            df.loc[code, u'今年前三月收益率'] = ser_3m_first[u'收益'] if ser_3m_first is not None and not ser_3m_first.empty else None
